...
**Название:** Team Assistant — Сервис коллективной работы на базе AI

**Описание:** Инверсия традиционной модели командной работы — не человек адаптируется к инструменту, а инструмент формируется вокруг естественных коммуникаций. Каждый участник команды взаимодействует со своим AI-ассистентом (голос, текст, Telegram, приложение). Ассистенты обмениваются информацией через общую шину данных (Data Bus). Структуры (задачи, сроки, связи) формируются автоматически средствами AI.

**Цель:** Решить проблему трения между реальной работой и инструментом. Существующие инструменты (Trello, Teamly, task trackers) требуют от пользователей адаптации к заданным онтологиям: проекты, задачи, workflow, доски. Люди устают от необходимости «жить» в очередной системе и часто откатываются к примитивным средствам (Google Docs, чаты). Team Assistant позволяет взаимодействовать через естественный диалог с минимальным порогом входа.
