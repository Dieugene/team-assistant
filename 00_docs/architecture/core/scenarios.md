...
sequenceDiagram
    ...
    Note over SIM: Инициализация сценария
    SIM ->> VU: создать профиль (роль, стиль, задачи)

    loop Симуляция диалога
        VU ->> DA: Message (как обычный User)
...
2. **Профиль VirtualUser определяет поведение.** Каждый VirtualUser имеет:
   - Роль в команде (разработчик, менеджер, дизайнер...)
   - Стиль коммуникации (краткий/развёрнутый, формальный/неформальный)
   - Набор текущих задач и контекст
   - Сценарий поведения (что хочет обсудить, какие вопросы задать)

3. **Осмысленность через контекст.** VirtualUser не просто генерирует
   случайные сообщения — он следует сценарию с конкретными задачами
   и реагирует на ответы ассистента.

4. **SIM управляет извне.** SIM запускает и останавливает VirtualUsers,
